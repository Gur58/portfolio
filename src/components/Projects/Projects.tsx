export const projects = [
    ...
    },
    {
        ...
        stack: ['Typescript', 'React', 'Formik', 'React-query', 'MicroFrontend Module-Federation']
    },
]
...
function Projects() {
    return (
        <div className="flex items-center flex-col">
            <div className="font-montserrat-black text-lg text-gray-700 xsm:text-2xl sm:text-3xl lg:text-4xl mb-10">Проекты</div>
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-1 lg:grid-cols-2 gap-4">
                {projects.map(item => (
                    <div className="border-[#15ccd5] border-solid border-2 rounded-lg p-5 md:p-10 shadow-md">
                        <h3 className="font-montserrat-black text-gray-700 text-lg md:text-2xl mb-3">{item.name}</h3>
                        <div className="flex flex-wrap gap-2">
                            {item.stack.map(item => (
                                <div className="bg-[#15ccd529] text-gray-700 text-xs md:text-sm px-3 rounded-xl">{item}</div>
                            ))}
                        </div>
                    </div>
                ))}
